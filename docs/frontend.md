...
- **Login**: `LoginPage` ahora tiene dos pasos. Si `POST /auth/login` devuelve `{status: "totp_required"}` en vez de abrir sesión directamente, el mismo formulario cambia a pedir un código (`POST /auth/2fa/verify-login`) sin volver a pedir email/contraseña — la sesión ya sabe, del lado del servidor, qué usuario tiene la contraseña correcta pendiente de confirmar. El campo de código admite tanto uno de la app (6 dígitos) como uno de recuperación (`XXXXX-XXXXX`); si se usa uno de recuperación, antes de entrar se muestra una pantalla intermedia con el aviso del backend ("te quedan N códigos sin usar") y un botón "Continuar", en vez de redirigir directamente — es la única forma de que ese aviso llegue a verse.
- **Configuración** (`/settings`, antes "próximamente"): activar el 2FA abre un diálogo que pide el código QR al servidor al abrirse (`POST /auth/2fa/setup`, una petición real al montar el diálogo — no hay `GET` natural para "generar un secreto nuevo", así que un efecto disparando la mutación al abrir es el sitio correcto para ello); al confirmarlo con un código, el mismo diálogo pasa a mostrar los 10 códigos de recuperación una única vez, con un aviso explícito de que no volverán a mostrarse. Desactivar pide contraseña y código (de la app o de recuperación) a la vez. Con el 2FA activo aparece además una fila "Códigos de recuperación" con cuántos quedan sin usar (`GET /auth/2fa/recovery-codes/status`, solo se pide si `totpEnabled`) y un botón para regenerarlos (pide contraseña, muestra el lote nuevo con el mismo componente `RecoveryCodesList` reutilizado del diálogo de activación).
...
El backend real de desarrollo del usuario (arrancado con `--watch` en su propia terminal) recompiló sin errores al guardar los cambios, en ambos pasos (2FA y luego códigos de recuperación). La verificación de extremo a extremo del flujo completo se hizo a nivel HTTP real, con cookies de sesión y cabecera CSRF igual que haría el navegador, en lugar de a través del navegador embebido: el puerto 5173 ya estaba ocupado por el servidor de desarrollo del propio usuario (confirmado por un `EADDRINUSE` al intentar arrancar uno propio), y usar ese proceso desde un navegador automatizado habría arriesgado cerrar o pisar la sesión real del usuario en su propio navegador. El usuario de prueba se creó y se borró de la base de datos al terminar en ambos casos.

## Añadido tras el cierre: ciclo de nómina y consejos en el dashboard

A petición del usuario: seguir el gasto desde el día que cobra la nómina en vez de por mes de calendario, con pagas extra contabilizadas aparte, y consejos de ahorro. Ver `docs/analytics-engine.md` y `docs/dashboard.md` para el diseño completo del backend.

- `DashboardPage` gana dos secciones nuevas, ambas alimentadas por la misma llamada a `GET /dashboard` de siempre (sin peticiones adicionales): una tarjeta **"Consejos"** (si `insights.length > 0`, con un icono y color por severidad — positiva/info/aviso, reutilizando el mismo patrón visual que las alertas de presupuestos/objetivos ya existentes) y una tarjeta **"Tu ciclo de nómina"** que solo aparece si `payCycle.hasSalaryData`; si no, se muestra una sugerencia explicando cómo activarlo (categorizar el ingreso como "Nómina").
- El hook `usePayCycleSummary` (`/analytics/pay-cycle`) queda disponible en `use-analytics.ts` para cuando se quiera llevar esta misma vista a Estadísticas, aunque el dashboard no lo necesita — ya recibe `payCycle` embebido en su propia respuesta.
- Tipo `Insight` (`severity`, `message`) nuevo en `types/insight.ts`, compartido por el dashboard sin necesitar saber nada de cómo se genera cada consejo — el texto ya viene completo desde el backend.

### Verificación

Probado en el navegador real con un usuario de prueba: una nómina de hace 40 días, otra de hace 10 (abre el ciclo actual), una paga extra dentro del ciclo actual y dos gastos categorizados iguales para forzar un aviso de incremento. El dashboard mostró correctamente: el ciclo empezando en la fecha real de la segunda nómina ("día 11 de este ciclo"), los 600 € de paga extra separados del ingreso base, la media de ciclos anteriores, y los dos consejos esperados (mayor gasto de la categoría, e incremento frente al ciclo anterior). Verificado también el caso sin nómina categorizada (usuario recién registrado, sin datos): no aparece la sección de consejos (nada que comentar todavía) y sí la sugerencia para activar el ciclo, sin ningún error. Los dos usuarios de prueba se borraron de la base de datos al terminar; el backend/frontend del propio usuario (puerto 3000, ya en marcha) no se tocó — se verificó contra su misma base de datos con usuarios de prueba aislados, y la parte visual se comprobó en una instancia de frontend propia en el puerto 5173 (libre en ese momento).
